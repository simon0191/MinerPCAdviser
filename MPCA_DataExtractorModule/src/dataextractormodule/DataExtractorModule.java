...
import dataProcessing.dataextractor.MpcaAmazonCommentsExtractor;
import dataProcessing.dataextractor.MpcaDataExtractor;
import dataProcessing.dataextractor.MpcaNeweggCommentsExtractor;
import dataProcessing.dataextractor.MpcaTigerDirectCommentsExtractor;
...
public class DataExtractorModule {
    ...
    public static void main(String[] args) throws Exception {
        MpcaIDataExtractor de = MpcaDataExtractor.getInstance();
        de.updateFromFiles("files.descriptor");
        //testExtractors();
    }
    
    private static void tigerDirectBrand(Document doc) {
        MpcaICommentsExtractor extractor = MpcaTigerDirectCommentsExtractor.getExtractor();
    ...
    private static void neweggBrand(Document doc) {
        MpcaICommentsExtractor extractor = MpcaNeweggCommentsExtractor.getExtractor();
        String res = extractor.brandExtractor(doc.select("fieldset").first());
        System.out.println(res);
    }
    
    private static void amazonBrand(Document doc) {
        MpcaICommentsExtractor extractor = MpcaAmazonCommentsExtractor.getExtractor();
